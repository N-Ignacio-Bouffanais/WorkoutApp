...
const Home: NextPage = () => {
    ...
    const [done, setDone] = useAppStore((state) => [state.done, state.toggleDone])
    //const { data, isError, isLoading, error } = api.exercise.getAll.useQuery()
                    ...
                    {(data || []).map((item: Exercise) => (
                        <div key={item.id} className='grid grid-cols-1 gap-2 mx-auto h-24 w-full lg:grid-cols-2' >
                            <div className='grid bg-green-600 h-24 w-full rounded-2xl py-4 p-2 items-center'>
                                <p className='text-center text-white font-semibold text-2xl' >{item.nameEx}</p>
                                <p className="text-center text-white font-semibold text-2xl">{item.reps}</p>
                                <p className="text-center text-white font-semibold text-2xl">{item.sets}</p>
                                <p className="text-center text-white font-semibold text-2xl">{item.weight}</p>
